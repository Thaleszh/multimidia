...
public class Bitmap {
    ...
    public void cuiftoBitmap(String filename) throws Exception {
      ...
      size_of_bitmap_file = 54 + height*width*3; // CabeÃ§alho de 54 bytes
      int i=0;
      ...
      offset_to_start_image = 54; // 54 bytes usado no cabeÃ§alho
    	...
    	// Primeiro é necessário gerar o raster YCbCr a partir do YCbCr do arquivo CUIF2
   		...
   		for (int i=1; i<=height; i++)
   	 		for (int j=1; j<=width; j++)
   	 			ycbcr[height-i][width-j][0] = (cuiffile[index++]&0xff);    
   		for (int i=1; i<=height; i++)
   	 		for (int j=1; j<=width; j++)
   	 			ycbcr[height-i][width-j][1] = (cuiffile[index++]&0xff);    
   		for (int i=1; i<=height; i++)
   	 		for (int j=1; j<=width; j++)
   	 			ycbcr[height-i][width-j][2] = (cuiffile[index++]&0xff);      
    ...
    private void readRaster(byte[] cuiffile, int index) {
      int r, g, b;
      raster=new int[height][width][3];
      ...
      for (int i=1; i<=height; i++){
        for (int j=1; j<=width; j++){
          ...
          raster[height-i][width-j][0] = r;
        }
      }
      for (int i=1; i<=height; i++){
        for (int j=1; j<=width; j++){
          g = cuiffile[index++]&0xff;
          raster[height-i][width-j][1] = g;
        }
      }
      for (int i=1; i<=height; i++){
        for (int j=1; j<=width; j++){
          b = cuiffile[index++]&0xff;
          raster[height-i][width-j][2] = b;
        }
      }

      // Escrita no array de bytes (bitmapfile) que representa em memÃ³ria o arquivo Bitmap
      int k=offset_to_start_image;
      for (int i=height-1; i>=0; i--){
      for (int j=width-1; j>=0; j--){
